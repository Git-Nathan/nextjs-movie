...
export default function SlideMedia({ data }: ISlideMediaProps) {
  ...
  return (
    <>
      <Swiper
        className={width >= 575 ? 'slide-desktop' : 'slide-mobile'}
        spaceBetween={width >= 575 ? 50 : 80}
        effect={'coverflow'}
        grabCursor={width <= 575 ? true : undefined}
        slidesPerView={width <= 575 ? 'auto' : undefined}
        coverflowEffect={
          width <= 575
            ? {
                rotate: 50,
        ...
        centeredSlides={true}
        loop={true}
        navigation={width >= 575 ? true : undefined}
        autoplay={{
          delay: 5000,
          ...
          <SwiperSlide key={item.id}>
            {width >= 575 ? (
              <div>
                ...
                ></div>
                <div
                  className={`homepage__text-banner absolute left-[5%] z-10 ${
